...
RUN_SPEED_KMPH = 20.0          # humuns average run speed(Km / Hour)
...
class SleepState:
    ...
    def do(boy):
        if boy.degree == 360:
            ...
            boy.degree += 1
        boy.frame = (boy.frame + FRAMES_PER_ACTION * ACTION_PER_TIME * game_framework.frame_time) % 8
        boy.gx = boy.radius * math.sin(boy.degree * boy.n) + boy.x
        boy.gy = boy.radius * math.cos(boy.degree * boy.n) + boy.y
        if boy.swich == 1:
           boy.a += 0.1
        else:
            boy.a -= 0.1
    ...
    def draw(boy):
        if boy.dir == 1:
            ...
            boy.image2.clip_draw(int(boy.frame) * 100, 300, 100, 100, boy.gx, boy.gy)
            boy.image2.opacify(boy.a)
        else:
